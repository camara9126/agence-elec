    ...
    <div class="facture-container">
        ...
        <div class="info-bar">
            ...
            <div class="info-right">
                <span class="facture-number">Facture N° {{ $facture->reference }}</span>
            </div>
        </div>
        ...
        <div class="totals-section">
            @if($entreprise->taux_tva > 0)
                <div class="total-line">
                    TVA ({{ $entreprise->taux_tva }}%) : {{ number_format($facture->total_tva ?? 0, 0, ',', ' ') }} FCFA
                </div>
                <div class="total-line">
                    Total HT : {{ number_format($facture->total ?? $totalGeneral, 0, ',', ' ') }} FCFA
                </div>
                <div class="total-ttc">
                    TOTAL TTC : {{ number_format($facture->total_ttc ?? ($totalGeneral * (1 + $entreprise->taux_tva/100)), 0, ',', ' ') }} FCFA
                </div>
            @else
                <div class="total-ttc">
                    TOTAL : {{ number_format($facture->total ?? $totalGeneral, 0, ',', ' ') }} FCFA
                </div>
            @endif
        </div>
        ...
        <div class="payment-info">
            @php
                $dernierPaiement = $facture->paiements->last();
            @endphp
            @if($dernierPaiement)
        ...
        <!--<div class="signature">
            <div class="signature-left">
                Par: {{ $facture->user->name ?? '_______________' }}
            </div>
            <div class="signature-right">
                Arrêtée la présente facture à la somme de : <strong>{{ number_format($facture->total_ttc ?? $totalGeneral, 0, ',', ' ') }} FCFA</strong>
            </div>
        </div>-->
