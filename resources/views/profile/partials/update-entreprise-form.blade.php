...
?>
 <section>
        ...
        <form method="post" action="{{ route('entreprise.update', $entreprise) }}" enctype="multipart/form-data" class="mt-6 space-y-6">
            @csrf
            @method('patch')
            ...
            <div>
                ...
                <x-text-input id="email" name="email" type="email" class="mt-1 block w-full" :value="old('email', $entreprise->email)" required autocomplete="email" />
            ...
            <div>
                ...
                <x-text-input id="" name="rccm" type="text" class="mt-1 block w-full" :value="old('rccm', $entreprise->rccm)" autofocus autocomplete="rccm" />
                <x-input-error class="mt-2" :messages="$errors->get('ninea')" />
            </div>

            <div>
                <x-input-label for="rib" :value="__('rib')" />
                <x-text-input id="" name="rib" type="text" class="mt-1 block w-full" :value="old('rib', $entreprise->rib)" autofocus autocomplete="rib" />
                <x-input-error class="mt-2" :messages="$errors->get('ninea')" />
            </div>
